...
* The digit recognition model may not work well with **handwritten digits**, since they were trained on a dataset with non-handwritten digits (mostly). Also if the puzzle image are **highly distoretd, blurred or noisy**, the algorithm  may fail to locate the puzzle and/or recognize the digits. 
